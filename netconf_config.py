...
def netconf_edit_config(m, xml, commit, retkwargs, datastore):
    ...
    try:
        ...
        m.edit_config(config=xml, target=datastore)
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            device_handler=dict(choices=['junos', 'csr', 'nexus', 'iosxr', 'iosxe', 'huawei', 'alu', 'h3c', 'hpcomware', 'default'], default='default'),
        ),
    ...
    nckwargs = dict(
        ...
        device_params={'name': module.params['device_handler']}
    )
